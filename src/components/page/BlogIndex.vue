<template >
    <div  class="table"  v-loading="loading">
        ...
        <el-table :data="tableData"  :row-class-name="tableRowClassName">
            ...
            <el-table-column    label="操作">
                <template slot-scope="scope">
                    ...
                    <el-button type="text" size="small" @click="blogEdit(scope.$index, scope.row)">编辑</el-button>
                    <el-button type="text" size="small" @click="blogDelete(scope.$index, scope.row)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog 
            ...
            <ul class="carList">
                <li><h3>ID</h3><P>{{onDate._id}}</P></li>
                <li><h3>栏目名称</h3><P>{{onDate.courseName}}</P></li>
                <li><h3>栏目描述</h3><P>{{onDate.courseDescribe}}</P></li>
                <li><h3>修改时间</h3><P>{{onDate.updateTime | formatDate}}</P></li>
            </ul>
    ...
    export default {
        ...
        methods: {
            ...
            blogDelete(index,row){
                console.log(index,row);
                console.log(`/api/admin/removeBlog?id=${row._id}`)
                axios.get(`/api/admin/removeBlog?id=${row._id}`).then((res) =>{
                    console.log(res);
                }).catch((err) => {
                    console.log(err);
                });
